...
- **Hybrid configuration** – `IsaacConfig` wraps both a `Siglip2`-style `IsaacVisionConfig` and a `Qwen3Config`. The merged
  config mirrors frequently accessed text attributes (hidden size, vocab size, rotary settings) while exposing vision-specific
  knobs such as `pixel_shuffle_scale_factor`, maximum patch budgets, and the `<image>` vision token.
- **Packed vision attention** – `IsaacVisionEncoder` keeps track of per-image patch lengths and uses specialized attention
  kernels with custom `AttentionMaskConverter` utilities so the decoder only pays attention to real patches while still
  benefiting from FlashAttention or SDPA.
...
- **Fast image pre-processing** – `IsaacImageProcessorFast` solves for the closest resolution that fits within the requested
...
`IsaacProcessor` currently supports `batch_size=1` and expects that every `<image>` token in the rendered prompt has a
