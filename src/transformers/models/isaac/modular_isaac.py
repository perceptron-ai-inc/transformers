...
class IsaacProcessor(ProcessorMixin):
    ...
    def _build_batch(
        ...
        for batch_idx, (text_value, sample_images) in enumerate(pairs):
            ...
            for item in items:
                ...
                if has_overlap and base_device is None:
                    base_device = (
                        image_inputs["vision_patches"].device if item["type"] == "image" else item["tokens"].device
                    )
        ...
        if image_inputs["vision_patches"].numel() == 0:
            ...
            vision_patches = image_inputs["vision_patches"].to(device=base_device)
            vision_patch_attention_mask = image_inputs["vision_patch_attention_mask"].to(device=base_device)
            vision_token_grids = vision_token_grids.to(device=base_device)
            vision_token_offsets = vision_token_offsets.to(device=base_device)
            vision_token_lengths = vision_token_lengths.to(device=base_device)
            vision_image_attention_mask = vision_image_attention_mask.to(device=base_device)
